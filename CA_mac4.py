'''Update: the cellular grids no need to show the Virus only state.
'''
import pygame
import random
import math
...
class Virus():
    ...
    def move(self,width,height):
        '''In One day, suppose the virus can randomly move
        to anywhere 
        '''
...
if __name__ == '__main__':
    ...
    zero_tile.fill((0,0,0))#black, nothing & dead
    one_tile = pygame.Surface((unit_size,unit_size))
    one_tile.fill((0,255,0)) # green, infected cell
    two_tile = pygame.Surface((unit_size,unit_size))
    two_tile.fill((255,0,0)) # red, target cell
